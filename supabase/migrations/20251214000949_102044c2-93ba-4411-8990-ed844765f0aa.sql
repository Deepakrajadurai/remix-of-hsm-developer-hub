...
-- RLS policy for user_roles table: only admins can view all roles
...
-- Add RLS policies to resources table for write operations
...
-- Also add similar protection for events table (it also only has SELECT policy)
